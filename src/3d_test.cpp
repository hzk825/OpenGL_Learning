...
int main() {
	...
	Shader mShader(".\\src\\GLSL\\3d.vs", ".\\src\\GLSL\\3d.fs");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		projection = glm::perspective(glm::radians(mCamera.Fov), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
		...
		for (unsigned int i = 0; i < 10; i++) {
			...
			model = glm::rotate(model, (float)glfwGetTime() * glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.4f));
